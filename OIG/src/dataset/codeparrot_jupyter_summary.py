...
class CodeParrortJupyterSummary(OIGBase):
    
    def __init__(self, output_path, hf_dataset_name="codeparrot/github-jupyter-parsed"):
        super().__init__()
    ...
    def __call__(self):
        ...
        for i in tqdm(range(len(self.dataset['train']))):
            data = self.dataset['train'][i]
            if not data['cells']:
                continue
            if "gpl" in data["license"]:
                continue
            if "unlicensed" in data["license"]:
                continue
            ...
            instance = {'subject': subj,'summary': summary, 'text': text}                
