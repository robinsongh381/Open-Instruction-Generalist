...
from src.dataset.ul2_oscar import *
...
class CUAD(OIGBase):
    ...
    @staticmethod
    def prepare_dataset():
        if not os.path.exists("data/cuad/CUADv1.json"):
            os.system('mkdir data/cuad')            
            os.system("wget https://github.com/TheAtticusProject/cuad/raw/main/data.zip")
            os.system("unzip data.zip -d data/cuad")
            os.sytem("rm -rf data.zip")

        dataset = json.load(open("data/cuad/CUADv1.json"))            
        return dataset
    ...
    def __call__(self):
        ...
        for idx in tqdm(range(len(dataset['data']))):
            ...
            for para in data['paragraphs']:
                ...
                if dialog_all:
                    instance = {"text": dialog_all, "metadata": {"source": "cuad"}}
