...
class FLANCoT(OIGBase):
    
    def __init__(self, output_path, hf_dataset_name=None):
        super().__init__()
        self.output_path = output_path
        self.dataset_path = "data/FLAN"
        
    def prepare_dataset(self):
        if not os.path.exists(self.dataset_path):
            os.makedirs("data", exist_ok=True)
            os.system(f"git clone https://github.com/google-research/FLAN {self.dataset_path}")
    
    def __call__(self):
        self.prepare_dataset()
        output = []        
        dataset_paths = glob.glob(f"{self.dataset_path}/flan/v2/cot_data/*.tsv")
        
        for idx in tqdm(range(len(dataset_paths))):
            ...
            with open(file, "rb") as in_data:
                for l in in_data:
                    ...
                    if "\n" in answer and answer.count("\n") > 1 and random.randint(0,1) == 0:
                        ...
                        instance = {'text': text, "metadata": {'source': 'flanv2_cot_'+dataset_name}}
