...
def detect_non_safe(text, url="", do_augment=False):
    ...
    text = text.lower()
    if "year old" in text: 
        text = text.replace("year old", "yo")
        
    if "years old" in text: 
        text = text.replace("years old", "yo")
...
class ImagePromptingInstructions(OIGBase):
    ...
    def __call__(self):
        ...
        for idx in tqdm(range(len(laion_img))):
            ...
            if label not in {"unknown", "unknown"}:
                continue
            ...
            if d:
                instance = {'text': d, 'metadata': {'source': 'laion_image_prompts'}}
