...
class UnifiedSKG(OIGBase):
    
    def __init__(self, output_path, hf_dataset_name=None):
        super().__init__()
        self.output_path = output_path
        self.dataset_path = "data/unifiedskg/unifiedskg.jsonl"
        
    def prepare_dataset(self):
        if not os.path.exists(self.dataset_path):
            os.makedirs("data/unifiedskg", exist_ok=True)
            # os.system(f"cp /content/drive/Shareddrives/LAION/OIG/unifiedskg.jsonl {self.dataset_path}")
            os.system(f"cp unifiedskg.jsonl {self.dataset_path}")
    ...
    def __call__(self):    
        ...
        seen = {}                
        output = []        
        with open(self.dataset_path) as inputs:
            for l in tqdm(inputs):
                ...
                text = text.strip()+"\n"
                instance = {'text': text, 'metadata': {'source': 'unifiedskg'}}
